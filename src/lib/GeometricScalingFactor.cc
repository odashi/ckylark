...
GeometricScalingFactor::GeometricScalingFactor(
    ...
    if (level != grammar.getLevel()) {
        throw runtime_error("GeometricScalingFactor::GeometricScalingFactor(): lexicon and grammar levels are mismatched.");
    }
    ...
    
    {
        ...
        // factor(w) = exp[ - sum_X[ P(X) log P(X -> w) ] ]
        ...
        for (int w = 0; w < num_words; ++w) {
            ...
            for (int tag = 0; tag < num_tags; ++tag) {
                ...
                for (int sub = 0; sub < num_sub; ++sub) {
                    ...
                    sum += semiterminal_cond_prob[mapping.getFinePos(tag, sub)] * log(score);
                }
            }
            
            lexicon_factor_[w] = exp(-sum);
        }
    }

    {
        // calculate grammar scaling factor
        // factor = exp[ - sum_X[ P(X) H(X) ] ]
        ...
        for (int i = 0; i < num_fine_pos; ++i) {
            if (sumprob[i] > 0.0) {
                e_entropy += inner_cond_prob[i] * entropy[i] / sumprob[i];
            }
        }

        grammar_factor_ = exp(-e_entropy);
    }
}
